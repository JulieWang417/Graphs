...
while len(visited) < len(world.rooms):
    ...
    path = []
    for i in exits:
        if i is not None and player.current_room.get_room_in_direction(i) not in visited: #if exit exists and we haven't visited
            """
    ...
    """
    if len(path) > 0:
        direction = path[random.randint(0, len(path) - 1)] # randomly choose  direction to go
        d.append(direction)
        player.travel(direction)
        traversal_path.append(direction)   
    else:
        prev_room = d.pop() ## Pop the top of the deque (Queue like)
        player.travel(backward[prev_room]) # the direction now change to backward[prev_room]
